# Codveda Python Internship – Python Development Tasks

This repository contains the tasks I completed during my **Python Development Internship** at **Codveda Technology**.  
It demonstrates my learning and skills in **basic, intermediate, and advanced Python programming**.

---

## 🔹 Level 1 – Basic Tasks

### 1. Number Guessing Game (`guessing_game.py`)
- Generates a random number between 1 and 100  
- User has limited attempts to guess the number  
- Program gives feedback if the guess is too high or too low  
- **Concepts:** random numbers, loops, conditionals, user input  

### 2. Simple Calculator (`simple_calculator.py`)
- Performs addition, subtraction, multiplication, and division  
- Handles division by zero with appropriate error messages  
- **Concepts:** functions, conditionals, user input  

---

## 🔹 Level 2 – Intermediate Tasks

### 1. To-Do List Application (`todo_list.py`)
- Command-line application to manage tasks  
- Features: Add task, View tasks, Mark as done, Delete task  
- Tasks are stored in a JSON file for persistence  
- **Concepts:** file handling, JSON, loops, conditionals, user interaction  

### 2. Weather Information Application (`api_integration.py`)
- Fetches real-time weather data for any city using **wttr.in API**  
- Displays temperature (°C / °F), weather condition, and humidity  
- Implements error handling for failed requests and invalid responses  
- Menu-driven interface for repeated queries  
- Free API (no signup required)  
- **Concepts:** API integration, JSON parsing, exception handling, user input  

---

## 🔹 Level 3 – Advanced Tasks

### 1. Django Web Application with Authentication
- Built a Django web application with user authentication  
- Implemented user registration, login, and logout  
- Secured passwords using Django’s built-in authentication system  
- Implemented user roles (admin and regular users)  
- Added password reset functionality  
- **Concepts:** Django framework, authentication, authorization, security  

### 2. Basic File Encryption & Decryption (`file_encrypt_decrypt.py`)
- Encrypts and decrypts text files using Caesar Cipher  
- Allows user to choose encryption or decryption  
- Saves encrypted and decrypted content to new files  
- Handles file not found errors gracefully  
- **Concepts:** file handling, encryption, exception handling  

---

## ▶ How to Run
1. Download or clone the repository  
2. Navigate to the required level folder  
3. Run the Python file using:
   ```bash
   python filename.py
🛠 Technologies Used
-Python 3
-JSON (for To-Do List storage)
-Random module
-Requests library (API integration)
-Django framework
-Basic file handling and string manipulation

👤 Author
Radhakrishna Kuchi
📧 Email: krishnaradha11114@gmail.com
🔗 LinkedIn: https://www.linkedin.com/in/radhakrishna-kuchi-310992298
🐙 GitHub: https://github.com/201183
